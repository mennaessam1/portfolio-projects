...
const BookingComponentForm = () => {
    ...
    const adults: String = searchParams.get("adults") || "1";
  ...
  const [numberOfPeople, setNumberOfPeople] = useState(Number(adults)); // Default to 1 person
...
const onSubmit = async (event: React.FormEvent) => {
  ...
  try {
      const response = await bookHotel(hotel, checkin, checkout, numberOfPeople, promocode, paymentMethod);
      toast.success("Booking Successful!", { id: toastId, duration: 1000 });
      localStorage.removeItem("selectedHotel");
      setTimeout(() => {
        router.push("/search-hotels"); // Redirect to transports page
      }, 1000); // 1-second delay
  } catch (error) {
            ...
            <div className="col-md-12">
              <div className="booking-form-wrapper mb-35">
                ...
                <div className="order-info-list">
                  <ul>
                    <li><span>Subtotal</span><span>${hotel.rawPrice * numberOfPeople}</span></li>
                    <li><span>Discount</span><span>     ${promocode && validPromo ? (((price/100)*(hotel.rawPrice * numberOfPeople)).toFixed(2)) : "0.00"}
                    </span></li>
                    <li><span>Total</span><span> ${promocode && validPromo ? (((1-price/100)*(hotel.rawPrice * numberOfPeople)).toFixed(2)) : "0.00"}</span></li>
                  </ul>
                </div>
